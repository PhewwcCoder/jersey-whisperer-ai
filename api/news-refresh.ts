...
import { resolveTier, TIER_LISTS, NATIONAL_TEAM_MAP, type NewsEventType, type Tier } from '../src/lib/news-score'
...
const HIGH_INTEREST_CLUBS = new Set([
  ...TIER_LISTS.clubs.most,
  ...TIER_LISTS.clubs.mid,
])
...
export async function refreshNewsEvents(): Promise<void> {
  ...
  for (const { name: clubName, id: teamId } of TRACKED_TEAM_IDS) {
    try {
      ...
      console.log(`[news-refresh] transfers team=${clubName} (call ${callCount}): ${data.response?.length ?? 0} player entries`)
      ...
      for (const entry of data.response ?? []) {
        ...
        for (const transfer of entry.transfers ?? []) {
          ...
          const tier = resolveTier(playerName) === 'low'
            ? (resolveTier(destClub) === 'low' ? 'low' : resolveTier(destClub))
  ...
  for (const leagueId of LEAGUE_IDS) {
    try {
      ...
      console.log(`[news-refresh] fixtures league=${leagueId} (call ${callCount}): ${data.response?.length ?? 0} fixtures`)
      ...
      for (const fixture of data.response ?? []) {
        ...
        if (HIGH_INTEREST_CLUBS.has(homeName) || HIGH_INTEREST_CLUBS.has(awayName)) {
          fixtureIdsToCheck.push({ fixtureId: fixture.fixture.id, fixtureDate: fixture.fixture.date })
        }
      }
    } catch (err) {
  ...
  for (const { fixtureId, fixtureDate } of fixturesToFetch) {
    try {
      ...
      for (const event of data.response ?? []) {
        ...
        const isTracked = ALL_TRACKED_PLAYERS_LOWER.some(p =>
          playerLower.includes(p) || p.includes(playerLower),
        )
  ...
  try {
    ...
    console.log(`[news-refresh] upserted ${rows.length} rows to news_events`)
  } catch (err) {
